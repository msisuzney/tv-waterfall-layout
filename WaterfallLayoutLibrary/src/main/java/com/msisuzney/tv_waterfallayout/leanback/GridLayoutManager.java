...
final class GridLayoutManager extends RecyclerView.LayoutManager {
    ...
    private boolean getAlignedPosition(View view, View childView, int[] deltas) {
        int scrollPrimary = getPrimarySystemScrollPosition(view);//这个View离顶部的距离
        if (DEBUG) Log.v(getTag(), "=======start=========");
        if (DEBUG) Log.v(getTag(), "childView@" + childView);
        ...
        if (DEBUG) Log.v(getTag(), "scrollPrimary3=" + scrollPrimary);
        if (DEBUG) Log.v(getTag(), "=======end=========");
        if (scrollPrimary != 0 || scrollSecondary != 0) {
            deltas[0] = scrollPrimary;
            deltas[1] = scrollSecondary;
            return true;
